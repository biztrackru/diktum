...
3. Убедиться, что `.env` содержит `HF_TOKEN`. Setup может принять token скрытым вводом и сохранить его локально.
4. Дважды кликнуть `Запустить Voice Recognizer.command`.
...
3. После успешной настройки дважды кликнуть `Запустить Voice Recognizer.command`.
4. Оставить открывшееся окно Terminal работать.
