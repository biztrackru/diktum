...
Очередь и история задач web-интерфейса сохраняются локально в `.cache/jobs/web_jobs.json`. После перезапуска сервера ожидающие задачи снова появятся в очереди, а задача, которая была в статусе `Выполняется` в момент остановки сервера, будет помечена как прерванная/ошибка вместо ложного вечного выполнения. Готовые результаты по-прежнему читаются из `outputs/`.
