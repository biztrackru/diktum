...
Ориентир текущего этапа: обычный пользователь Mac получает папку/пакет Диктум, запускает setup и затем обрабатывает длинные диктофонные записи локально без помощи разработчика и без Terminal.
...
- Публичное имя продукта обновлено на `Диктум`; launchers, web UI, trial pack и промо-материалы синхронизированы с новым брендом.
