...
## 2026-06-27 long chunk punctuation follow-up

Повторный прогон `Модуль 3, день 2` показал, что проблема была не только в stale artifacts. GigaSTT корректно запускался с `--punctuation on`, но на ASR-чанках по 3600 секунд возвращал почти полностью lowercase/raw text. Локальный smoke на одном и том же начальном фрагменте:

- 60/180/300/600 секунд: пунктуация и заглавные буквы есть;
- 900 секунд: пунктуация и заглавные буквы исчезают.

Исправление:

- дефолт `--asr-chunk-seconds` снижен с 3600 до 600 секунд;
- chunking теперь включается для файлов длиннее chunk size, а не только для файлов длиннее hard limit GigaSTT;
- ASR JSON metadata поднята до версии 2 и хранит chunking-параметры;
- chunk artifact paths теперь содержат start/duration, чтобы старые `part-001` на 3600 секунд не могли подменить новый 600-секундный chunk.

Практический эффект: старые ASR JSON версии 1 и старые часовые chunks автоматически считаются stale. Для проблемных файлов можно физически удалить старые `outputs/pipeline/<stem>*` и `.cache/audio/<stem>*`, но после этого изменения это уже не обязательно для корректного нового прогона.
